    ...
    const fetchEvents = () => {
        ...
        AxiosInstance.get<EventsData[]>('/event?type=career')
            .then(response => {
                ...
                const careerEvents = response.data.filter(event => event.type === 'career');
    ...
    const createEvent = () => {
        ...
        AxiosInstance.post('/event', newEvent)
            .then((response) => {
                ...
                setEvent({ title: '', description: '', location: '', type: EventType.CAREER });
            })
            .catch(error => {
                console.error('Error creating event:', error);
                if (error.response && error.response.data) {
                    console.error('Backend error response:', error.response.data);
                    setCreateEventError(error.response.data.message || 'Failed to create event. Please try again.');
                } else {
...
export const useUpdateEvent = (setEvents: React.Dispatch<React.SetStateAction<EventsData[]>>) => {
    ...
    const updateEvent = async () => {
        ...
        try {
            const response = await AxiosInstance.patch(`/event/${editingEvent._id}`, updatedEvent);
    ...
    return { editingEvent, setEditingEvent, showForm, handleEditChange, handleEditClick, updateEvent, toggleForm };
};
