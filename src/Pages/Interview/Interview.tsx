            ...
            <div key={phase} className={style.kanbanColumn}>
              ...
              <Droppable droppableId={phase}>
                {(provided) => (
                  ...
                  >
                    {getInterviewsByPhase(phase).length === 0 ? (
                      <div>No interviews in this phase.</div>
                    ) : (
                      getInterviewsByPhase(phase).map((interview, index) => (
                        <Draggable key={interview._id} draggableId={interview._id} index={index}>
                          {(provided) => (
                            <div
                              ref={provided.innerRef}
                            ...
                            >
                              <h6
                                onClick={() => handleNavigateToProfile(interview._id)}
                              ...
                              {phase !== 'applicant' && (
                                <>
                                  {interview.firstInterviewDate && phase !== 'Employed' && (
                                    <p>Interview Date: {formatDate(interview.firstInterviewDate)}</p>
                                  )}
                                  <p>Email: {interview.email || 'N/A'}</p>
                                  <p>Phone: {interview.phoneNumber || 'N/A'}</p>
                                  <p>Notes: {interview.notes || 'N/A'}</p>
                                  {phase === 'first' && interview.secondInterviewDate && (
                                    <p>Second Interview Date: {formatDate(interview.secondInterviewDate)}</p>
                                  )}
                                </>
                              )}
